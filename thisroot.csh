...
set path = (/opt/root/bin $path)

if ($?LD_LIBRARY_PATH) then
   setenv LD_LIBRARY_PATH /opt/root/lib/root:$LD_LIBRARY_PATH      # Linux, ELF HP-UX
else
   setenv LD_LIBRARY_PATH /opt/root/lib/root
endif

if ($?DYLD_LIBRARY_PATH) then
   setenv DYLD_LIBRARY_PATH /opt/root/lib/root:$DYLD_LIBRARY_PATH  # Mac OS X
else
   setenv DYLD_LIBRARY_PATH /opt/root/lib/root
endif

if ($?SHLIB_PATH) then
   setenv SHLIB_PATH /opt/root/lib/root:$SHLIB_PATH                # legacy HP-UX
else
   setenv SHLIB_PATH /opt/root/lib/root
endif

if ($?LIBPATH) then
   setenv LIBPATH /opt/root/lib/root:$LIBPATH                      # AIX
else
   setenv LIBPATH /opt/root/lib/root
endif

if ($?PYTHONPATH) then
   setenv PYTHONPATH /opt/root/lib/root:$PYTHONPATH
else
   setenv PYTHONPATH /opt/root/lib/root
endif

if ($?MANPATH) then
   setenv MANPATH `dirname /opt/root/share/man/man1`:$MANPATH
else
   setenv MANPATH `dirname /opt/root/share/man/man1`:$default_manpath
endif
