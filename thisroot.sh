...
if [ -z "${PATH}" ]; then
   PATH=/opt/root/bin; export PATH
else
   PATH=/opt/root/bin:$PATH; export PATH
fi

if [ -z "${LD_LIBRARY_PATH}" ]; then
   LD_LIBRARY_PATH=/opt/root/lib/root; export LD_LIBRARY_PATH       # Linux, ELF HP-UX
else
   LD_LIBRARY_PATH=/opt/root/lib/root:$LD_LIBRARY_PATH; export LD_LIBRARY_PATH
fi

if [ -z "${DYLD_LIBRARY_PATH}" ]; then
   DYLD_LIBRARY_PATH=/opt/root/lib/root; export DYLD_LIBRARY_PATH   # Mac OS X
else
   DYLD_LIBRARY_PATH=/opt/root/lib/root:$DYLD_LIBRARY_PATH; export DYLD_LIBRARY_PATH
fi

if [ -z "${SHLIB_PATH}" ]; then
   SHLIB_PATH=/opt/root/lib/root; export SHLIB_PATH                 # legacy HP-UX
else
   SHLIB_PATH=/opt/root/lib/root:$SHLIB_PATH; export SHLIB_PATH
fi

if [ -z "${LIBPATH}" ]; then
   LIBPATH=/opt/root/lib/root; export LIBPATH                       # AIX
else
   LIBPATH=/opt/root/lib/root:$LIBPATH; export LIBPATH
fi

if [ -z "${PYTHONPATH}" ]; then
   PYTHONPATH=/opt/root/lib/root; export PYTHONPATH
else
   PYTHONPATH=/opt/root/lib/root:$PYTHONPATH; export PYTHONPATH
fi

if [ -z "${MANPATH}" ]; then
   MANPATH=`dirname /opt/root/share/man/man1`:${default_manpath}; export MANPATH
else
   MANPATH=`dirname /opt/root/share/man/man1`:$MANPATH; export MANPATH
fi
